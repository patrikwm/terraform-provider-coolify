...
### Example: Server with Validation Waiter

```terraform
# Wait for server validation (is_reachable && is_usable) before marking as complete
...
resource "coolify_server" "validated" {
  ...
  description      = "Automatically validated"
  ip               = "192.168.1.100"
  port             = 22
  user             = "root"
  private_key_uuid = coolify_private_key.example.uuid
  ...
  # Optional: customize validation timeout (default: 3 minutes)
  timeouts {
    create = "5m"
  }
}

# Now you can deploy services immediately without manual intervention
...
- `wait_for_validation` (Boolean) Wait for server validation (is_reachable && is_usable) to complete during creation. Defaults to false.
