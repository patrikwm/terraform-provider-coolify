---
# generated by https://github.com/hashicorp/terraform-plugin-docs
page_title: "coolify_service_envs Resource - coolify"
subcategory: ""
description: |-
  Create, read, update, and delete Service environment variables.
---

# coolify_service_envs (Resource)

Create, read, update, and delete Service environment variables.

## Example Usage

```terraform
resource "coolify_service_envs" "example" {
  # Service UUID
  uuid = "i0800ok00gcww840kk8sok0s"

  env {
    key   = "key1"
    value = "value1"
  }

  env {
    key   = "key2"
    value = "value2"
  }
}
```

### Example: Auto-Redeploy on Configuration Change

```terraform
# Automatically restart the service when environment variables change
# This is useful for services that need to reload configuration on env changes

resource "coolify_service_envs" "api_config" {
  uuid = coolify_service.critical_app.uuid

  # Enable automatic redeployment when any env var changes
  redeploy_on_change = true

  env {
    key   = "API_KEY"
    value = var.api_key
  }

  env {
    key   = "DATABASE_URL"
    value = "postgresql://user:pass@db:5432/mydb"
  }

  env {
    key   = "FEATURE_FLAGS"
    value = jsonencode({
      new_ui = true
      beta_api = false
    })
  }
}

# When you update any env var (e.g., change api_key), Terraform will:
# 1. Update the environment variables
# 2. Automatically trigger a service restart
# 3. Wait for the restart to complete (if wait_for_deployment is enabled on the service)
```

### Example: Without Auto-Redeploy (Default)

```terraform
# By default, changing env vars only updates the configuration
# You must manually redeploy or restart the service for changes to take effect

resource "coolify_service_envs" "static_config" {
  uuid = coolify_service.example.uuid

  # redeploy_on_change defaults to false

  env {
    key   = "CONFIG_OPTION"
    value = "static_value"
  }
}
```

<!-- schema generated by tfplugindocs -->
## Schema

### Required

- `uuid` (String) UUID of the service.

### Optional

- `env` (Block List) Environment variable to set. (see [below for nested schema](#nestedblock--env))
- `redeploy_on_change` (Boolean) Automatically redeploy the service when environment variables change. Defaults to false.

<a id="nestedblock--env"></a>
### Nested Schema for `env`

Required:

- `key` (String) The key of the environment variable.
- `value` (String) The value of the environment variable.

Optional:

- `is_build_time` (Boolean) The flag to indicate if the environment variable is used in build time.
- `is_literal` (Boolean) The flag to indicate if the environment variable is a literal, nothing espaced.
- `is_multiline` (Boolean) The flag to indicate if the environment variable is multiline.
- `is_preview` (Boolean, Deprecated) Not supported on services and should not be used.
- `is_shown_once` (Boolean) The flag to indicate if the environment variable's value is shown on the UI.

Read-Only:

- `uuid` (String) UUID of the service.

## Import

Import is supported using the following syntax:

The [`terraform import` command](https://developer.hashicorp.com/terraform/cli/commands/import) can be used, for example:

```shell
terraform import coolify_service_envs.example <service_uuid>
```
